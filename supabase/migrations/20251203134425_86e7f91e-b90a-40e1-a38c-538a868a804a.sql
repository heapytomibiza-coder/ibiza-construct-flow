...
-- Admin function to remove logistics questions from question packs
-- These questions are handled in Step 5 (LogisticsStep) and shouldn't appear in Step 4
...
BEGIN
  ...
  ALTER TABLE question_packs DISABLE TRIGGER prevent_approved_mutation;
  ...
  LOOP
    ...
    -- Handle case where all questions might be filtered (shouldn't happen but safety check)
    ...
    IF before_count > after_count THEN
      ...
      micro_slug := pack_record.micro_slug;
  ...
  ALTER TABLE question_packs ENABLE TRIGGER prevent_approved_mutation;
...
-- Grant execute to authenticated users (admins would call this)
GRANT EXECUTE ON FUNCTION admin_remove_logistics_questions() TO authenticated;

COMMENT ON FUNCTION admin_remove_logistics_questions IS 
'Removes logistics-related questions (urgency, timeline, budget, etc.) from approved question packs. 
These questions are handled in Step 5 (LogisticsStep) and should not appear in Step 4 micro-specific questions.';
