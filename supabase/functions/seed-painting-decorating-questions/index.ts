...
import { paintingDecoratingQuestionPacks } from '../_shared/paintingDecoratingQuestionPacks.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Starting to seed ${paintingDecoratingQuestionPacks.length} Painting & Decorating packs...`);

    let inserted = 0;
    let skipped = 0;
    const errors: Array<{ slug: string; error: string }> = [];

    for (const pack of paintingDecoratingQuestionPacks) {
      try {
        ...
        const content = {
          ...
          name: micro.name,
          slug: pack.microSlug,
          i18nPrefix: `painting-decorating.${pack.subcategorySlug}.${pack.microSlug}`,
    ...
    return new Response(JSON.stringify({
      success: true,
      total: paintingDecoratingQuestionPacks.length,
      inserted,
      skipped,
