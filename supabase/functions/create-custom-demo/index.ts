...
serve(async (req) => {
  ...
  try {
    ...
    if (!accounts || !Array.isArray(accounts)) {
      return new Response(
        JSON.stringify({ error: 'Invalid request: accounts array required' }),
    ...
    for (const account of accounts) {
      ...
      const userId = account.userId || crypto.randomUUID();
      ...
      const { error: profileError } = await supabaseAdmin
        ...
        .insert({
          ...
          preferred_language: account.language || 'en'
        });
    ...
    return new Response(
      JSON.stringify({ 
        message: 'Custom demo accounts creation completed',
        results 
      }),
      { 
    ...
    console.error('Error in create-custom-demo:', error);
