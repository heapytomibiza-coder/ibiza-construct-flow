...
import { legalRegulatoryQuestionPacks } from '../_shared/legalRegulatoryQuestionPacks.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`⚖️ Seeding ${legalRegulatoryQuestionPacks.length} Legal & Regulatory question packs...`);
    ...
    for (const pack of legalRegulatoryQuestionPacks) {
      try {
    ...
    return new Response(
      JSON.stringify({
        message: 'Legal & Regulatory question packs seeded',
        results,
      }),
      {
